...
class SelectionGNN(nn.Module):
    ...
    def __init__(self,
        ...
        self.dimLayersMLPs = dimLayersMLPs
        ...
        for l in range(self.L):
            ...
            #\\ Pooling
            if self.coarsening:
                gfl.append(self.rho(self.N[l], self.N[l+1], self.alpha[l]))#self.rho(self.alpha[l]))
            else:
        ...
        if len(self.dimLayersMLPs) > 0: # Maybe we don't want to MLP anything
            # The first layer has to connect whatever was left of the graph
            # signal, flattened.
            for mlp_type, dimLayersMLP in self.dimLayersMLPs.items():
                fc = []
                ...
                for l in range(len(dimLayersMLP)-1):
                 ...
                 # And we're done
                self.MLPs[mlp_type] = nn.Sequential(*fc)

                if mlp_type == 'cls-bld-flr':
                    self.MLPBldFlr = nn.Sequential(*fc)

                elif mlp_type == 'reg-coords':
                    self.MLPCoords = nn.Sequential(*fc)

                elif mlp_type == 'cls-refloc':
                    self.MLPRefLoc = nn.Sequential(*fc)
    ...
    def splitForward(self, x):
        ...
        result = {}
        if self.MLPBldFlr is not None:
            result['cls-bld-flr'] = self.MLPBldFlr(yFlat)

        if self.MLPCoords is not None:
            result['reg-coords'] = self.MLPCoords(yFlat)

        if self.MLPRefLoc is not None:
            result['cls-refloc'] = self.MLPRefLoc(yFlat)
        return result #(self.MLPs[mlp_type](yFlat) for mlp_type in self.dimLayersMLPs)
...
class GraphConvolutionAttentionNetwork(nn.Module):
    ...
    def __init__(self,
        ...
        self.MLPs = {}
        if len(self.dimLayersMLPs) > 0: # Maybe we don't want to MLP anything
            for mlp_type in dimLayersMLPs:
                dimLayersMLP = dimLayersMLPs[mlp_type]
                fc = []
                # The first layer has to connect whatever was left of the graph
                # signal, flattened.
                ...
                for l in range(len(dimLayersMLP)-1):
                    # Add the nonlinearity because there's another linear layer
                    # coming
                    fc.append(self.sigmaMLP())#self.sigmaMLP())
                    # And add the linear layer
                    fc.append(nn.Linear(dimLayersMLP[l], dimLayersMLP[l+1],
                                        bias = self.bias))
                    if dropout:
    ...
    def forward(self, x):
        ...
        # And, feed it into the MLP
        #return {mlp: self.MLPs[mlp](y) for mlp in self.MLPs}
        result = {}
        if self.MLPBldFlr is not None:
            result['cls-bld-flr'] = self.MLPBldFlr(y)

        if self.MLPCoords is not None:
            result['reg-coords'] = self.MLPCoords(y)

        if self.MLPRefLoc is not None:
            result['cls-refloc'] = self.MLPRefLoc(y)
        return result
        # If self.MLP is a sequential on an empty list it just does nothing.
